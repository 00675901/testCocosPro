...
void PickCoinGame::operationCoin() {
    ...
    if ((tCount > 0 && playerLayer->canBeOperation(tCount)) || currCoinCount <= 3) {
...
void PickCoinGame::operationCoinByAI() {
    ...
    if (currCoinCount > 3) {
        x = rand() % 3 + 1;
        if (!playerLayer->canBeOperation(x)) {
            if (x == 1) {
                x++;
            } else if (x == 3) {
                x--;
